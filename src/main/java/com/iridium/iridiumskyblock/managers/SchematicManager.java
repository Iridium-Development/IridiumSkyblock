...
public class SchematicManager {
    ...
    public SchematicManager() {
        ...
        try {
            if (IridiumSkyblock.getInstance().getDatabaseManager().getSchematicTableManager().getList().size() == 0) {
                addDefaultSchematics();
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        this.defaultSchematic = IridiumSkyblock.getInstance().getDatabaseManager().getSchematicTableManager().getList().get(0);
    ...
    private void addDefaultSchematics() throws IOException {
        ...
        IridiumSkyblock.getInstance().getDatabaseManager().getSchematicTableManager().getList().add(new SchematicData("island", new BufferedReader(new InputStreamReader(inputStream)).readLine()));
    }
    ...
    public CompletableFuture<Void> pasteSchematic(final Island island, final World world, final String schematicID, final int delay) {
        ...
        Schematic schematic = IridiumSkyblock.getInstance().getDatabaseManager().getSchematicTableManager().getList().stream().filter(schematicData -> schematicData.getId().equalsIgnoreCase(schematicID)).findFirst().orElse(defaultSchematic).getSchematic();
    ...
    private void saveSchematic(String name, Schematic schematic) {
        ...
        IridiumSkyblock.getInstance().getDatabaseManager().getSchematicTableManager().getList().removeIf(s -> s.getId().equals(name));
        IridiumSkyblock.getInstance().getDatabaseManager().getSchematicTableManager().getList().add(schematicData);
    }
