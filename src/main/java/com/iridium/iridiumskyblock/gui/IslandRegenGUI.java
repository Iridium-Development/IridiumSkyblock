...
public class IslandRegenGUI extends SchematicGUI {
    ...
    public void selectSchematic(Map.Entry<String, Schematics.SchematicConfig> schematicConfig) {
        ...
        if (island.isPresent()) {
            new BukkitRunnable() {
                @Override
                public void run() {
                    new ConfirmationGUI(IridiumSkyblock.getInstance().getConfiguration().confirmation.islandRegenPrompt,
                            () -> {
    ...
    private CooldownProvider<CommandSender> getCooldownProvider() {
