...
public class BlockFormListener implements Listener {
    ...
    public void onBlockForm(BlockFormEvent event) {
        ...
        // Custom basalt generators should only work in nether IF netherExclusiveGenerator = true
        if (newMaterial == XMaterial.COBBLESTONE
                || newMaterial == XMaterial.STONE
                || (newMaterial ==
                XMaterial.BASALT && ((event.getBlock().getLocation().getWorld().getEnvironment() == World.Environment.NETHER)
                        && IridiumSkyblock.getInstance().getConfiguration().netherExclusiveGenerator))
                || (newMaterial ==
                XMaterial.BASALT && (!IridiumSkyblock.getInstance().getConfiguration().netherExclusiveGenerator))) {
