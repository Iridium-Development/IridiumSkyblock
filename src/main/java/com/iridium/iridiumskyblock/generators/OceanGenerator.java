...
public class OceanGenerator extends IridiumChunkGenerator {
    ...
    public void generateWater(World world, int x, int z) {
        ...
        // Generate layer of bedrock
        WaterLayersGenerator.generateBedrockLayer(world, x, z, minHeightWorld);

        // Generate gravel layer
        WaterLayersGenerator.generateGravelLayer(world, x, z, bottomMaterial, currentFloorHeight, minHeightWorld);

        // Generate sand on top of gravel
        WaterLayersGenerator.generateSandonGravel(world, x, z, topMaterial, currentFloorHeight);

        // Generate water or lava on top of the floor
        WaterLayersGenerator.generateWateronFloor(world, x, z, waterHeight, currentFloorHeight);

        // Replace everything else with air
        replaceAllwithAir(world, x, z, waterHeight);
    }

    private void generateGravelLayer(World world, int x, int z, XMaterial bottomMaterial, int currentFloorHeight, int minHeightWorld) {
    ...
    private void generateBedrockLayer(World world, int x, int z, int minHeightWorld) {
    ...
    private void generateWateronFloor(World world, int x, int z, int waterHeight, int currentFloorHeight) {
        XMaterial oceanMaterial = world.getEnvironment() == Environment.NETHER ? XMaterial.LAVA : XMaterial.WATER;
        for (int y = currentFloorHeight + 1; y <= waterHeight; y++) {
            Block block = world.getBlockAt(x, y, z);
    ...
    private void generateSandonGravel(World world, int x, int z, XMaterial topMaterial, int currentFloorHeight) {
