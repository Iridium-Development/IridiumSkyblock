...
public class DemoteCommand extends Command {
    ...
    public DemoteCommand() {
        super(Collections.singletonList("demote"), "Demote a user", "%prefix% &7/is demote <player>", "", true,
                Duration.ZERO);
    }
        ...
        if (offlinePlayerUser != null) {
            ...
            for (User member : island.get().getMembers()) {
                ...
                if (islandMember.equals(player)) {
                    islandMember.sendMessage(StringUtils.color(IridiumSkyblock.getInstance().getMessages().demotedPlayer
            ...
            IslandLog islandLog = new IslandLog(island.get(), LogAction.USER_DEMOTED, user, offlinePlayerUser, 0,
                    nextRank.getDisplayName());
